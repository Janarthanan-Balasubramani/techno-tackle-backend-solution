...
const createStore = async (store: any) => {
  const isStoreNameALreadyExist =
    await StoreRepository.checkIsStoreNameAlreadyTaken(store);
  if (isStoreNameALreadyExist) {
    throw new HttpError(HttpStatus.BAD_REQUEST, 'Store name already exist');
  }
...
const updateStore = async (store: any) => {
  const isStoreNameALreadyExist =
    await StoreRepository.checkIsStoreNameAlreadyTakenWithDifferentId(store);
  if (isStoreNameALreadyExist) {
    throw new HttpError(
